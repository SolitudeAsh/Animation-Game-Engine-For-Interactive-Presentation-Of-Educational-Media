...
var board_size = 8  # Default size, must be a power of 2
var cell_size = 64
var board = []
var tromino_count = 0
var colors = [
...
var solving_steps = []
var current_step = 0
var selected_square = Vector2(-1, -1)  # Stores the currently selected square
...
@onready var input_n_field = $CanvasLayer/InputNField  # LineEdit for 'n' value input
...
func setup_ui():
	...
	# Initialize input field for 'n' value
	if input_n_field:
		input_n_field.connect("text_submitted", _on_n_value_submitted)
	else:
		print("InputNField node not found.")  # Debugging
...
func _draw():
	...
	for y in range(board_size):
		for x in range(board_size):
			...
			if selected_square == Vector2(x, y):
				draw_rect(rect, Color(1.0, 1.0, 0.0, 0.5), true)

func _input(event):
	if event is InputEventMouseButton and event.pressed:
		# Detect click on board and select square
		...
		if grid_x >= 0 and grid_x < board_size and grid_y >= 0 and grid_y < board_size:
			selected_square = Vector2(grid_x, grid_y)
			print("Selected square: ", selected_square)
			# Place a tromino in the selected square if it's valid (for testing)
			board[grid_y][grid_x] = tromino_count + 1  # Example: placing a tromino number
			tromino_count += 1
			queue_redraw()

func _on_n_value_submitted(value: String):
	# Convert the string value from the input field to an integer
	var new_n = int(value)
	if new_n > 0 and is_power_of_two(new_n):  # Ensure n is a power of 2
		board_size = new_n
		reset_puzzle()
